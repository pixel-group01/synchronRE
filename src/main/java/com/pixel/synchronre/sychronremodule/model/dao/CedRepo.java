...
public interface CedRepo extends JpaRepository<Cedente, Long>
{
    ...
    @Query("""
        select new com.pixel.synchronre.sychronremodule.model.dto.cedente.ReadCedenteDTO(c.cedId, c.cedNomFiliale, c.cedSigleFiliale, c.cedTel, c.cedEmail, 
        c.cedAdressePostale,c.cedFax, c.cedSituationGeo, c.cedStatut.staLibelle) 
        from Cedente c where (locate(upper(coalesce(?1, '') ), upper(cast(function('strip_accents',  coalesce(c.cedEmail, '') ) as string)) ) >0 
