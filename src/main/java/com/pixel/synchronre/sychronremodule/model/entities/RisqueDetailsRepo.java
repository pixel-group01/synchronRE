...
{
    @Query("select r from RisqueCouvertDetails r where r.risqueCouvert.risqueId = ?1 and r.couverture.couId = ?2")
    RisqueCouvertDetails findByRisqueIdAndSousCouId(Long risqueId, Long couId);

    @Query("select (count(r.risqueDetailsId) > 0) from RisqueCouvertDetails r where r.risqueCouvert.risqueId = ?1 and r.couverture.couId = ?2")
