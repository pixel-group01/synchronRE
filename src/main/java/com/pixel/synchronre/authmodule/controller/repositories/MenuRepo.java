...
public interface MenuRepo extends JpaRepository<Menu, Long>
{
    ...
    @Query("select (count(m.menuId) > 0) from Menu m where m.menuCode = ?1")
    boolean existsByMenuCode(String menuCode);

    @Query("select (count(m.menuId) > 0) from Menu m where m.name = ?1")
    boolean existsByName(String name);

    @Query("select prv.privilegeId from AppPrivilege prv where prv.privilegeCode in ?1")
    Set<Long> getMenuPrvIdsByMenuCodes(Set<String> menuCodes);
