           ...
           value = """
                WITH encaissement AS (
                    ...
                    reversement AS (
                        ...
                        FROM Vreversement rev
                        WHERE rev.date_reglement BETWEEN :debut AND :fin
                        GROUP BY rev.aff_id, rev.ces_id
                        ORDER BY rev.aff_id, rev.ces_id
                    )
                select new com.pixel.synchronre.statsmodule.model.dtos.StatChiffreAffaireParPeriodeDTO(
                ...
                from affaire aff
                    ...
                    LEFT JOIN Vplacement pla ON db.deb_rep = pla.rep_id
            ...
            """)
    List<StatChiffreAffaireParPeriodeDTO> getStatsChiffreAffaire(@Param(value = "exeCode") Long exeCode,
