...
public interface ChiffreAffaireRepo extends JpaRepository<Affaire, Long>
{
    @Query(nativeQuery = true,
            ...
            (WITH encaissement AS (
                    ...
                    FROM V_paiement p
                    WHERE p.date_reglement BETWEEN coalesce(:debut, p.date_reglement) AND coalesce(:fin, p.date_reglement)
                    GROUP BY p.aff_id
                ),
                     reversement AS (
                         SELECT
                             rev.aff_id as aff_id,
                             rev.ces_id as ces_id,
                             MAX(rev.date_reglement) AS date_reversement,
                             SUM(rev.montant_reverse) AS montant_reverse
                         FROM V_reversement rev
                         WHERE rev.date_reglement BETWEEN coalesce (:debut, rev.date_reglement) AND coalesce (:fin, rev.date_reglement)
                ...
                where aff.exe_code = coalesce(:exeCode, aff.exe_code) and ced.ced_id = coalesce(:cedId, ced.ced_id) and ces.ces_id = coalesce(:cesId, ces.ces_id)) k
            """,
           value = """
                WITH encaissement AS (
                    SELECT
